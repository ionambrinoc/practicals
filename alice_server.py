...
def clientthread(conn):
	...
	urB = long(conn.recv(128))	# message 2: Bob's corresponding D-H messages.
	urBp= long(conn.recv(128))  # reconverted into (useful) longs.
	...
	yrppb = long(conn.recv(128))# message 4, from Bob: y^(-r''_b)
	...
	bobCheck = long(conn.recv(128)) # message 6, Bob's analogous to 5.

	t = aliceCheck * bobCheck % p

	rBp = long(conn.recv(128))		# reverse message order; Bob discloses one of the 
	if (u^rBp) % p != urBp:
